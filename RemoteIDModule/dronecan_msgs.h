/*
  ...
  本文件由 libcanard DSDL 编译器自动生成，包含所有 DroneCAN/UAVCAN 消息类型的
  C 语言数据结构和编解码函数声明。

  包含的消息类别：
  - dronecan.remoteid.*：RemoteID 远程识别消息（BasicID/Location/SelfID 等）
  - uavcan.protocol.*：UAVCAN 协议消息（NodeStatus/GetNodeInfo/参数/调试 等）
  - uavcan.equipment.*：设备消息（ESC/GNSS/电源/传感器 等）
  - 第三方设备消息（HobbyWing ESC/HiMark 舵机/Volz 舵机 等）

  注意：不要手动修改此文件，它由 DSDL 工具链自动生成。
 */
#pragma once
